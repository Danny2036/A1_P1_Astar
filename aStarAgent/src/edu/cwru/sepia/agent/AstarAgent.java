...
public class AstarAgent extends Agent {
	...
	class MapLocation implements Comparable<MapLocation>{
		...
		/**
		 * The mapLocation used to reach this MapLocation
		 */
		public MapLocation cameFrom;

		/**
		 * The function value for A* f(n) = g(n) + h(n)
		 */
		public int fValue;

		/**
		 * The path cost till this point
		 */
		public int gValue;
		...
		public MapLocation(int x, int y, MapLocation cameFrom, float cost, int f, int g){
			//Call to the provided constructor
	...
	public Map<Integer, Action> initialStep(State.StateView newstate,
		...
		if (!newstate.getUnit(footmanID).getTemplateView().getName()
				.equals("Footman")) {
		...
		for (Integer unitID : enemyUnitIDs) {
			...
			String unitType = tempUnit.getTemplateView().getName()
					.toLowerCase();
	...
	public Map<Integer, Action> middleStep(State.StateView newstate,
		...
		if (nextLoc != null && (footmanX != nextLoc.x || footmanY != nextLoc.y)) {
			...
			actions.put(footmanID,
					Action.createPrimitiveMove(footmanID, nextDirection));
		} else {
	...
	public void terminalStep(State.StateView newstate,
		...
		System.out.println("Total time: "
				+ (totalExecutionTime + totalPlanTime) / 1e9);
	}
	 ...
	 *
	 * This method should return true when the path needs to be replanned and
	 * false otherwise. This will be necessary on the dynamic map where the
	...
	/**
	 * This method is implemented for you. You should look at it to see examples
	 * of how to find units and resources in Sepia.
	 *
	 * @param state
	 * @return
	 */
	private Stack<MapLocation> findPath(State.StateView state) {
		...
		return AstarSearch(startLoc, goalLoc, state.getXExtent(),
				state.getYExtent(), footmanLoc, resourceLocations);
	}

	/**
	 * This is the method you will implement for the assignment. Your
	 * implementation will use the A* algorithm to compute the optimum path from
	 * the start position to a position adjacent to the goal position.
	 *
	 * You will return a Stack of positions with the top of the stack being the
	 * first space to move to and the bottom of the stack being the last space
	 * to move to. If there is no path to the townhall then return null from the
	 * method and the agent will print a message and do nothing. The code to
	 * execute the plan is provided for you in the middleStep method.
	 *
	 ...
	 *
	 * xExtent would be 5 for this map with valid X coordinates in the range of
	 * [0, 4] x=0 is the left most column and x=4 is the right most column
	 *
	 * yExtent would be 3 for this map with valid Y coordinates in the range of
	 * [0, 2] y=0 is the top most row and y=2 is the bottom most row
	 *
	...
	private Stack<MapLocation> AstarSearch(MapLocation start, MapLocation goal, int xExtent, int yExtent, MapLocation enemyFootmanLoc, Set<MapLocation> resourceLocations) {
		...
		PriorityQueue<MapLocation> unVisistedLocation = new PriorityQueue<MapLocation>();

		unVisistedLocation.add(new MapLocation(start.x, start.y, start.cameFrom, 0, chebyShev(start, goal), 0));
		MapLocation currentLocation;

		while(!unVisistedLocation.isEmpty()){
			currentLocation = unVisistedLocation.remove();

			if(currentLocation.equals(goal)){
				System.out.println("Goal discovered. Hurrah!");
				return getFullPath(start, currentLocation);
			}

			visitedLocations.add(currentLocation);
			Set<MapLocation> nextLocations = getNextLocations(visitedLocations, currentLocation, enemyFootmanLoc, xExtent, yExtent, resourceLocations);
			for(MapLocation neighbor: nextLocations){
				if(!unVisistedLocation.contains(neighbor)){
					int gOfN = currentLocation.gValue;
					unVisistedLocation.add(new MapLocation(neighbor.x, neighbor.y, currentLocation, 0, gOfN + chebyShev(neighbor, goal), gOfN));
				}
			}
		}
	...
	/**
	 * The heuristic to be used by A* for finding the fastest path
	 * @param start The starting location of the heurisitic 
	 * @param goal The goal location of the heuristic
	 * @return
	 */
	...
	private Stack<MapLocation> getFullPath(MapLocation start,
			MapLocation currentLocation) {
		Stack<MapLocation> fullPath = new Stack<MapLocation>();
		fullPath.push(currentLocation);
		MapLocation currentSpot = currentLocation;
		while(!start.equals(currentSpot)){
			fullPath.push(currentSpot);
			currentSpot = currentSpot.cameFrom;
		}
		return fullPath;
	}

	private Set<MapLocation> getNextLocations(HashSet<MapLocation> visitedLocations, MapLocation currentLocation,
			MapLocation enemyFootmanLoc, int xExtent, int yExtent, Set<MapLocation> resourceLocations) {
		HashSet<MapLocation> validLocations = new HashSet<MapLocation>();
		for(int i = -1; i < 2; i++){
			for(int j = -1; j < 2; j++){
	...
	private boolean isValid(HashSet<MapLocation> visitedLocations, MapLocation location, MapLocation enemyFootmanLoc,
			MapLocation currentLocation, Set<MapLocation> resourceLocations, int xExtent, int yExtent) {
		boolean validity = true;
		if(resourceLocations.contains(location)){
			validity = false;
		} else if (location.equals(currentLocation)){
			validity = false;
		} else if (location.x < 0 || location.y < 0 || location.x > xExtent || location.y > yExtent){
			validity = false;
		} else if (location.equals(enemyFootmanLoc)){
			validity = false;
